...
class MetricsCollector {
  ...
  constructor() {
    ...
    this.responseTimes = [];
    this.maxResponseTimeHistory = 100; // 최대 100개의 응답 시간 기록 (메모리 최적화)
...
export const healthCheck = (req: Request, res: Response) => {
  try {
    ...
    // 헬스 상태 계산 - 시스템 전체 메모리 기준으로 계산
    const totalSystemMemory = os.totalmem();
    const memoryUsagePercent = (memoryUsage.rss / totalSystemMemory) * 100;
    ...
    // 메모리 사용률 체크 (시스템 메모리 기준, 70% 초과 시 경고)
    ...
    // 메모리 사용률이 85% 초과하거나 에러율이 50% 초과 시 unhealthy
    if (memoryUsagePercent > 85 || errorRate > 50) {
      status = 'unhealthy';
    }
...
const monitorThresholds = () => {
  ...
  thresholdMonitorInterval = setInterval(() => {
    ...
    // 메모리 사용률 80% 초과 시 경고 (조기 감지로 최적화)
    if (memoryUsagePercent > 80) {
      logger.warn('High memory usage detected', {
        memoryUsagePercent: Math.round(memoryUsagePercent * 100) / 100,
        heapUsed: metrics.memoryUsageMB.heapUsed,
        heapTotal: metrics.memoryUsageMB.heapTotal,
        rss: metrics.memoryUsageMB.rss
      });
      
      // 메모리 사용률이 높을 때 가비지 컵렉션 수행
      if (memoryUsagePercent > 85 && global.gc) {
        global.gc();
        logger.info('Manual garbage collection triggered due to high memory usage');
      }
    }
