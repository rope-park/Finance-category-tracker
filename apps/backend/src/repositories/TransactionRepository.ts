...
export class TransactionRepository extends BaseRepository {
   ...
   * ID로 거래를 조회합니다
   */
  async findById(id: number, userId: number): Promise<TransactionRecord | null> {
   ...
   * 거래를 업데이트합니다
   */
  async updateTransaction(id: number, userId: number, data: UpdateTransactionData): Promise<TransactionRecord | null> {
  ...
  async findWithFilters(
    ...
    if (filters.transaction_type) conditions.transaction_type = filters.transaction_type;
    ...
    if (filters.description_contains) {
      ...
      values.push(`%${filters.description_contains}%`);
    }
  ...
  async findByCategory(
    ...
    return await this.findWithFilters(
      { user_id: userId, category_key: categoryKey },
      limit,
      offset
    );
  }
      ...
      for (const transaction of transactions) {
        const result = await client.query(
          `INSERT INTO ${this.tableName} 
           (user_id, category_key, transaction_type, amount, description, transaction_date)
           VALUES ($1, $2, $3, $4, $5, $6)
           RETURNING *`,
          [
            transaction.user_id,
