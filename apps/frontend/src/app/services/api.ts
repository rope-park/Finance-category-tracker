...
class FinanceTrackerAPI {
  ...
  async getRecommendedCategories(ageGroup: string, jobGroup?: string): Promise<ApiResponse<{ recommended_categories: string[] }>> {
    ...
    return this.request(`/recommend/categories?${params.toString()}`);
  }
  ...
  async getDemoTransactions(params?: {
    ...
    const queryString = queryParams.toString();
    const endpoint = queryString ? `/transactions/demo?${queryString}` : '/transactions/demo';
