...
import { getCategoryIcon, getCategoryName, formatCurrency } from '../../../index';
...
export const DashboardPage: React.FC<DashboardPageProps> = ({ onTabChange }) => {
  ...
  const trend = months.map(month => {
    ...
    const income = monthTx.filter(t => t.transaction_type === 'income').reduce((sum, t) => sum + t.amount, 0);
    const expense = monthTx.filter(t => t.transaction_type === 'expense').reduce((sum, t) => sum + t.amount, 0);
  ...
  const catTx = transactions.filter(t => {
    const d = new Date(t.date);
    return t.transaction_type === 'expense' && d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth();
  });
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ textAlign: 'right' }}>
                      <AmountDisplay
                        ...
                        type={transaction.transaction_type === 'transfer' ? 'neutral' : transaction.transaction_type} 
                        size="sm"
                        style={{
                          fontFamily: "'Noto Sans KR', sans-serif"
                        }}
